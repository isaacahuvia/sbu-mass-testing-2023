...
out %>%
  filter(rq == "1b") %>%
  select(dv, control, estimate, p.adj) %>%
  pivot_wider(
    names_from = control,
    values_from = c(estimate, p.adj)
  ) %>%
...
fitted_data_combined %>%
  filter(self_id == "Yes") %>% # Can also try faceted graph, but it looks confusing!
  ggplot() +
...
### Analysis 2B: Is depression self-labeling associated wtih certain coping strategies?
